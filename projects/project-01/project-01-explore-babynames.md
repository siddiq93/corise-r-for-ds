...
tbl_names_popular |> 
  ...
  labs(
    title = 'Most popular names of the decade (2011-2020)',
    subtitle = 'Top 5 male and female names by number of births',
    caption = 'Source: US Social Security Administration',
    x = 'Number of births (in thousands)',
    y = 'Name'
  ) +
...
```
    ...
    #>     year sex   first_letter nb_births pct_births
    #>    <dbl> <chr> <chr>            <dbl>      <dbl>
    #>  1  2020 F     A               274456     0.171 
    #>  2  2020 F     B                50182     0.0312
    #>  3  2020 F     C                93005     0.0578
    #>  4  2020 F     D                38936     0.0242
    #>  5  2020 F     E               152416     0.0947
    #>  6  2020 F     F                16130     0.0100
    #>  7  2020 F     G                44041     0.0274
    #>  8  2020 F     H                54874     0.0341
    #>  9  2020 F     I                42399     0.0263
    #> 10  2020 F     J                73307     0.0456
    #> # ℹ 42 more rows
...
``` r
  ...
  labs(
    ...
    subtitle = "Proportions of births across first letter of name for 2020, by sex",
...
plot_trends_in_letter <- function(my_letter) {
  tbl_names_by_letter |> 
    ...
    labs(
      ...
      subtitle = "Proportions of births across years for names starting with {my_letter}, by sex",
      caption = "Data source: U.S. Social Security Administration",
      x = "Year",
      y = 'Proportion of births'
    ) +
...
``` r
  ...
  ggplot(aes(x = last_letter, y = first_letter, fill = pct_births)) +
  # Add a `geom_tile` layer with fill mapped to pct_births
  geom_tile() +
  # Add labels (title, subtitle, x, y, fill)
  labs(
    title = "Distribution of Percentage of Births by First and Last Letters",
...
tbl_names_vowel_consonant <- tbl_names |> 
  ...
  unite(first_last, c(first_letter_type, last_letter_type), sep = "") 
```
...
``` r
  ...
  # Add labels (title, subtitle, caption, x, y)
  labs(
    title = "Percentage of Births by Combination of First and Last Letter Types, Faceted by Sex",
    subtitle = "Proportions of births across combinations of first and last letter types, by sex",
    x = "Year",
    y = "Proportion of births",
    fill = "First and Last Letter Types"
  ) +
