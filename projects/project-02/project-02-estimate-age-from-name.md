...
file_name <- here::here("data/lifetables.csv.gz")
tbl_lifetables <- readr::read_csv(file_name, show_col_types = FALSE)
...
plot_estimated_age <- function(tbl, my_name, my_sex) {
  tbl |> 
    ...
    # Add a column layer with y = nb_alive, and appropriate colors
      geom_col(aes(y = nb_alive), fill = "darkred") +
    # Add a line layer with y = nb_births, and appropriate colors
      geom_line(aes(y = nb_births), color = "darkorange") + 
